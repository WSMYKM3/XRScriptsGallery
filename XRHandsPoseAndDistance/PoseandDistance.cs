//no timer, but works with Pose and distance detetction
    ...
    public class WSMStaticHandGesture : MonoBehaviour
    {
        ...
        void OnJointsUpdated(XRHandJointsUpdatedEventArgs eventArgs)
        {
            ...
            if (m_IsTrackingMovement)
            {
                ...
                Debug.Log($"Current movement distance: {m_CurrentMovementDistance:F3}m");
            }

            if (!m_PerformedTriggered && detected)
            {
                var holdTimer = Time.timeSinceLevelLoad - m_HoldStartTime;
                if (holdTimer > m_MinimumHoldTime && m_CurrentMovementDistance >= m_MinimumMovementDistance)
                {
                    Debug.Log($"Gesture performed! Movement distance: {m_CurrentMovementDistance:F3}m");
